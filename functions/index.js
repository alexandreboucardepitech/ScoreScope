...
// Fonction principale pour récupérer les matchs des 2 prochaines semaines
exports.fetchNextTwoWeeksMatches = onSchedule("0 0 * * *", async () => {
  ...
  try {
    ...
    for (const leagueId of competitions) {
      ...
      for (const matchData of data) {
        ...
        const matchDocRef = db.collection("matches").doc(id);
        ...
        if (!matchDoc.exists) {
          ...
          const matchObj = {
            ...
            date: new Date(matchData.fixture.timestamp * 1000).toISOString(),
          ...
          console.log("Match ajouté :", id);
        } else {
          console.log("Match déjà existant :", id);
        }
      }
    }
